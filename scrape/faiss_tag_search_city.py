...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Embed tourism tags with LM Studio and search top matches in a FAISS index (city dataset)."
    )
    parser.add_argument(
        "--tags",
        nargs="+",
    ...
    parser.add_argument(
        "--index-path",
        default="wikivoyage_city.index",
        help="Path to FAISS index file.",
    )
    parser.add_argument(
        "--texts-path",
        default="wikivoyage_city_texts.json",
        help="Path to JSON list of chunk texts.",
    )
    parser.add_argument(
        "--metadata-path",
        default="wikivoyage_city_metadata.json",
    ...
    parser.add_argument(
        "--article-avg",
        action="store_true",
        help="Compute city-level average distance across all listings in each city.",
    )
    parser.add_argument(
        "--article-top-k",
        type=int,
        default=20,
        help="How many top cities to return when --article-avg is enabled.",
    )
    parser.add_argument(
        "--article-entry-top-k",
        type=int,
        default=5,
        help="How many top entries to return for each selected city.",
    )
    parser.add_argument(
        "--count-reward-alpha",
        type=float,
        default=0.1,
        help="Strength of city count reward in weighted distance (higher rewards more listings).",
    )
    parser.add_argument(
        "--count-reward-tau",
        type=float,
...
def merge_results(tag_results, texts, metadata, tag_weight_by_tag):
    merged = {}
...
def print_results(results, top_k, show_chars):
    ...
    for rank, item in enumerate(results[:top_k], start=1):
        ...
        city = meta.get("city", "") or meta.get("article", "")
        ...
        print(f"   title={title} | city={city} | country={country} | type={listing_type}")
        print(f"   text={preview}")


def get_group_name(meta):
...
def print_article_summaries(article_summaries, show_chars):
    print("\nCity average distance results:")
    if not article_summaries:
        print("No city summaries available.")
        return

    for rank, item in enumerate(article_summaries, start=1):
        print(
            f"#{rank} city={item['article']} weighted={item['weighted_distance']:.4f} "
        ...
        for entry_rank, entry in enumerate(item.get("top_entries", []), start=1):
            ...
            print(
                ...
                f"title={title} type={listing_type} country={country}"
            )
            print(f"     text={preview}")
...
def main():
    ...
    merged = merge_results(tag_results, texts, metadata, tag_weight_by_tag)
    ...
    if args.json_output:
        ...
        with open(output_path, "w", encoding="utf-8") as handle:
            payload = {
                ...
                "city_results": article_summaries if args.article_avg else [],
                "city_results_all": full_article_summaries if args.article_avg else [],
            }
