...
# Default input is the CSV in workspace root when this script is under scrape/.
...
def save_outputs(output_prefix="wikivoyage", output_dir=None, *, texts_jsonl_path=None, metadata_jsonl_path=None):
...
def build_and_embed(input_csv, *, output_dir, output_prefix, max_rows=None):
    ...
    def flush_batch(text_handle, meta_handle):
        ...
        print(f"Embedded chunks: {embedded_chunks}/{chunks_prepared}")
...
def run_pipeline(input_csv=DEFAULT_INPUT_CSV, max_rows=None, output_prefix="wikivoyage", output_dir=None):
    reset_storage()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Load Wikivoyage CSV data, chunk text, embed with LM Studio, and build a FAISS index."
    )
    parser.add_argument(
        "--input-csv",
        default=str(DEFAULT_INPUT_CSV),
        help="Path to wikivoyage CSV file (UTF-8).",
    )
    parser.add_argument(
        "--max-rows",
        type=int,
